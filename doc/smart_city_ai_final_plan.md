...
초기 단계에서는 실제 도로 신호기, 실제 CCTV 스트리밍, 긴급차량 통신망과 직접 연동하지 않는다. 대신 샘플 영상, 이미지, 시뮬레이션 데이터, 가상 센서 데이터를 기반으로 교차로 상태를 분석하고, 분석 결과를 교통 이벤트로 저장한 뒤, AI Agent가 현재 상황을 요약하고 신호 전환 또는 우선 제어 시나리오를 추천한다.
...
| LLM | 자연어 질의응답, 이벤트 요약, 리포트 생성 |
| RAG | 이벤트 로그, 운영 기준 문서, 공공데이터 기반 근거 검색 |
| AI Agent | 분석 결과, 이벤트 로그, 추천 규칙을 연결해 신호제어 추천 생성 |

이 구조는 단일 AI 모델 데모가 아니라, 공공 서비스에서 AI를 실제 업무 흐름에 연결하는 형태를 보여준다.
...
| LLM/VLM | OpenAI API | 장면 설명, 질의응답, 요약, 리포트 생성 |
...
| 시뮬레이션 데이터 | 방향별 차량 수, 보행자 요청, 신호 상태를 JSON으로 생성 |
| 긴급차량 이벤트 | MVP에서는 시뮬레이션 이벤트로 생성 |
...
| LLM/VLM | OpenAI API | 장면 설명, 질의응답, 리포트 생성 |
...
| 5주차 | 이벤트 생성 로직, 이벤트 타임라인, 혼잡도 계산 구현 |
| 6주차 | 신호제어 추천 로직, 디지털 트윈 시뮬레이션 구현 |
| 7주차 | 자연어 질의응답, RAG 검색, 10분 리포트 생성 구현 |
| 8주차 | 통합 테스트, 비용 점검, 발표자료, 데모 시나리오 정리 |
